			...
			for (int i = 0; i != 2; ++i)
			{
				*find_vacant_object(boss.get_bullets_ref())
					...
					[=](Bullet& _b)->void
				{
					...
					auto omega = 0.1; if (count > 30 && count < 100){ omega += (count - 30) * static_cast<Real>(0.002); }
...
void MyGameProject::BlackWingsImple::draw_charactor(void) const
{
	...
	for (const auto& p : trail)
	{
		...
		FunctionWrapper::DrawCircle(p.x(), p.y(), 14, gp::red.get());
	}

	FunctionWrapper::DrawCircle(boss.pos().x(), boss.pos().y(), 14, gp::black.get());

	if (wave == 1)
	{
		FunctionWrapper::DrawCircle(p1.x(), p1.y(), 14, gp::red.get());
		FunctionWrapper::DrawCircle(p2.x(), p2.y(), 14, gp::red.get());
	}
	if (white_out_flag)
	{
