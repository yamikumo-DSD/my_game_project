...
namespace
{
	...
	constexpr size_t MAX_SHOT_NUM = 64;
	constexpr size_t MAX_MOB_ENEMY_NUM = 33;
	...
	inline void dynamic_object_updater(DynamicObjectPtrContainer& _objects)
	{
		for (auto& object : _objects)
		{
			if (object)
			{
				if (object->get_flag()){ object->update(); }
				else{ object.reset(); }
			}
		}
	}
...
MyGameProject::PlayGame::PlayGame(void)
	...
	code_list()
{
	player = gp::make_smart<Player>(*device, std::bind(std::mem_fn(&SEManager::erect_play_flag_of), se_manager.get(), std::placeholders::_1));
